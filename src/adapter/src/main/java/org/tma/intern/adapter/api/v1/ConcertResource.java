...
public class ConcertResource extends BaseResource {
    ...
    @APIResponse(responseCode = "201", description = "Success",
            content = @Content(schema = @Schema(implementation = String.class)))
    public Uni<RestResponse<CommonResponse<Long>>> create(ConcertRequest.Body body) {
        return concertService.create(body)
                .onItem().transform(id -> RestResponse.ResponseBuilder.ok(
    ...
    @APIResponse(responseCode = "201", description = "Success",
            content = @Content(schema = @Schema(implementation = String.class)))
    public Uni<RestResponse<CommonResponse<Long>>> update(Long id, ConcertRequest.Body body) {
    ...
    @APIResponse(responseCode = "201", description = "Success",
            content = @Content(schema = @Schema(implementation = String.class)))
    public Uni<RestResponse<CommonResponse<Long>>> softDelete(Long id) {
        return concertService.softDelete(id)
                .onItem().transform(resultId ->
                        RestResponse.ResponseBuilder.ok(
                                CommonResponse.<Long>builder()
                                        .message(messages.get("Action.Success", "delete", "concert"))
                                        .data(id).build(),
                                MediaType.APPLICATION_JSON).build());
    ...
    public Uni<RestResponse<CommonResponse<List<Long>>>> seed(int count) {
        return concertService.seedData(count)
                .collect().asList()
                .map(ids -> RestResponse.ok(CommonResponse.<List<Long>>builder().data(ids).build()));
    }
